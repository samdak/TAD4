...
public class Asteroids extends Applet implements Runnable, KeyListener {
  ...
  public void updateUfo() {
    ...
    if (ufo.active) {
      ...
      if (ufo.active) {
        ...
        ufo.render();
        for (i = 0; i < MAX_SHOTS; i++)
          if (Photons.photons[i].active && ufo.isColliding(Photons.photons[i])) {
            if (sound)
              crashSound.play();
            explode(ufo);
            stopUfo();
  ...
  public void updateMissle() {
    ...
    if (missle.active) {
      if (--missleCounter <= 0)
        ...
        missle.render();
        for (i = 0; i < MAX_SHOTS; i++)
          if (Photons.photons[i].active && missle.isColliding(Photons.photons[i])) {
            if (sound)
              crashSound.play();
            explode(missle);
            stopMissle();
    ...
    for (i = 0; i < MAX_ROCKS; i++)
      if (asteroids[i].active) {
        ...
        for (j = 0; j < MAX_SHOTS; j++)
          if (Photons.photons[j].active && asteroids[i].active && asteroids[i].isColliding(Photons.photons[j])) {
            asteroidsLeft--;
            asteroids[i].active = false;
            Photons.photons[j].active = false;
            if (sound)
              explosionSound.play();
            explode(asteroids[i]);
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.getKeyChar() == ' ' && ship.active) {
      ...
      Photons.photonTime = System.currentTimeMillis();
      Photons.photonIndex++;
      if (Photons.photonIndex >= MAX_SHOTS)
        Photons.photonIndex = 0;
      Photons.photons[Photons.photonIndex].active = true;
      Photons.photons[Photons.photonIndex].x = ship.x;
      Photons.photons[Photons.photonIndex].y = ship.y;
      Photons.photons[Photons.photonIndex].deltaX = 2 * MAX_ROCK_SPEED * -Math.sin(ship.angle);
      Photons.photons[Photons.photonIndex].deltaY = 2 * MAX_ROCK_SPEED *  Math.cos(ship.angle);
    }
